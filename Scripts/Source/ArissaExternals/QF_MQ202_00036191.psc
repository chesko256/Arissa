...
Alias_Gissur.GetRef().Disable()
Alias_Shavari.GetRef().Disable()
...
kmyQuest.CheckForGissurOverhearing(Alias_Dirge.GetActorRef())
SetStage(55)
;END CODE
EndFunction
;END FRAGMENT
...
kmyQuest.CheckForGissurOverhearing(Alias_Vekel.GetActorRef())
;END CODE
EndFunction
;END FRAGMENT
...
if gissur.IsDead() == 0
	...
	gissur.moveTo(Alias_MainAmbush1.GetRef())
endif
; Esbern outro scene finished
EsbernExplainScene.Stop()
; make him a teammate for trip to Riverwood
Alias_Esbern.GetActorRef().setPlayerTeammate(true, false)
...
Actor gissur = Alias_Gissur.GetActorRef()
gissur.AddToFaction(EnemyofPlayerFaction)
gissur.SetActorValue("Aggression", 1)
